...
public class RestaurantMenuImpl implements IRestaurantMenu {
	...
	public List<TablesResponce> getTables(String restaurantId) {
		List<TablesResponce> tablesResponses1 = new ArrayList<TablesResponce>();
		DynamoDB dynamoDB = DBService.getDBConnection();
		...
		valueMap.put(":letter1", restaurantId + "_");
		QuerySpec spec = new QuerySpec()
				.withKeyConditionExpression("restaurantId = :v_id and begins_with(tableId,:letter1)")
				.withValueMap(valueMap);
		...
		while (iterator.hasNext()) {
			Item item = iterator.next();
			TablesResponce tablesResponce = new TablesResponce();
			tablesResponce.setBookedDate(item.getString("bookedDate"));
			tablesResponce.setRestaurantId(item.getString("restaurantId"));
			tablesResponce.setSeats(item.getString("seats"));
			tablesResponce.setMinSeats(item.getString("minSeats"));
			tablesResponce.setTableStatus(item.getString("tableStatus"));
			tablesResponce.setTableId(item.getString("tableId"));
			tablesResponce.setTableName(item.getString("tableName"));
			tablesResponce.setBookedTime(item.getString("bookedTime"));	
			tablesResponce.setBookedSeats(item.getString("bookedSeats"));	
			tablesResponses1.add(tablesResponce);		
		}
		return tablesResponses1;
	...
	public WebsiteMenuCategoryWithItemsResponse getMenuItemsWithRestaurantId(String restaurantId) {
		StringBuilder monToFri = new StringBuilder();
		StringBuilder satToSun = new StringBuilder();
		...
		Consumer<Item> action4 = new Consumer<Item>() {
			...
			public void accept(Item t3) {				
			 monToFri.append(t3.getString("monToFri"));	
				satToSun.append(t3.getString("satToSun"));	
				country.append(t3.getString("country"));
		...
		websiteMenuCategoryWithItemsResponse.setMonToFri(monToFri.toString());
		websiteMenuCategoryWithItemsResponse.setSatToSun(satToSun.toString());
		websiteMenuCategoryWithItemsResponse.setCountry(country.toString());
		System.out.println(menuCategoryItems);
	...
	public static void main(String[] args) {
		RestaurantMenuImpl restaurantMenuImpl = new RestaurantMenuImpl();
		restaurantMenuImpl.getTables("23");
	}
